...
yeePay.prototype.parseReturn = function(data,encryptkey){
	...
	try{
		...
		var errData = {code:-5};
		try{
			errData['err'] = JSON.parse(jsonStr);
			errData['msg'] = errData['err']['error_msg'];
		}catch(err){
			errData['err'] = jsonStr;
			errData['msg'] = jsonStr;
		}
 ...
 * @return object
 */
yeePay.prototype.doWithdraw = function(obj,cb){
	var request = require('request');
	var querystring = require('querystring');
	request.post({url:this.config.API_TZT_withdraw_Base_Url,body:JSON.stringify(obj)},function(error, response, body){
...
yeePay.prototype._post = function(obj,cb){
	obj.sign = this.RSASign(obj);
	var requestData = {
		merchantaccount:obj.merchantaccount,
		encryptkey:encodeURIComponent(this.getEncryptkey()),
		data:encodeURIComponent(this.AESEncryptRequest(obj))
	}
	var request = require('request');
	request.post({url:this.config.API_TZT_Check_Bankcard,body:JSON.stringify(requestData)},function(error, response, body){
		if(!error && response.statusCode === 200){
			cb(null,JSON.parse(body));
		}else{
			cb(error, response, body);
...
yeePay.prototype.drawRecord = function(obj,cb){
	...
	if(util.isNotEmptyObj(obj)){
		if(!util.checkQueryWithdrawParam(obj)){
			return cb(msg);
		}
		var queryObj = _.extend(this.getWithdrawDefaultConfig(this.config),obj);
